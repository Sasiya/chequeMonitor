...
export default class ChequePassed extends Component {
  constructor(props) {
    super(props);
  ...
  getTotal() {
    ...
    let pendingCheque = chequeData.filter(row => {
      return row.chequeStatus == 'passed';
    });
    if (billDate != '') {
  ...
  renderDataList() {
    ...
    const sortedActivities = pendingCheque.sort(
      (a, b) => b.chequeDate - a.chequeDate,
    );
    ...
    return (
      <View style={styles.mainConteiner}>
        ...
        <Dialog.Container visible={dialog}>
          ...
          <Dialog.Description>
            Do you want to set pending this cheque ?
          </Dialog.Description>
          <Dialog.Button
            label="Cancel"
            onPress={() => this.setState({dialog: !this.state.dialog})}
          />
          <Dialog.Button label="Yes" onPress={() => this.pendingCheque()} />
        </Dialog.Container>
      </View>
    );
  }
